Public Class p31WorksheetDL
    ...
    Public Function GetList_Pivot(rows As List(Of BO.PivotRowColumnField), cols As List(Of BO.PivotRowColumnField), sums As List(Of BO.PivotSumField), mq As BO.myQueryP31) As IEnumerable(Of BO.PivotRecord)
        Dim s As New System.Text.StringBuilder, x As Integer = 0
        s.Append("SELECT ")

        For Each c In rows
            If x > 0 Then s.Append(",")
            s.Append(c.SelectField & " AS Row" & (x + 1).ToString)
            x += 1
        Next
        Dim y As Integer = 0
        For Each c In cols
