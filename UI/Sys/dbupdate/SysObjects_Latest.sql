...
AS
BEGIN
  ---vrací èárkou oddìlené obsazení klientských rolí v klientovi @p28id
...
if @x29id<>328
 begin		---vyjímku má pouze entita Klient
  if @x38id is null and @isdraft=0
   RETURN('')	---pokud na vstupu chybí èíselná øada x38id, pak musí být záznam v režimu DRAFT. U normálních záznamù nesmí x38id chybìt
 end
...
if @x29id=141 and @isdraft=1	---projekt DRAFT, x38id mùže být prázdné
...
if @j11ids is null
 begin	---seznam týmù s úèastí osoby
...
if @ref_pid is not null
 set @err_ret='Minimálnì jeden typ klienta je svázaný s touto èíselnou øadou.'
...
if @ref_pid is not null and @x29id=328
 set @err_ret='Tato role je již obsazena v minimálnì jednom záznamu klienta ('+dbo.GetObjectAlias('p28',@x69recordpid)+')'

if @ref_pid is not null and @x29id=356
 set @err_ret='Tato role je již obsazena v minimálnì jednom úkolu ('+dbo.GetObjectAlias('p56',@x69recordpid)+')'

if @ref_pid is not null and @x29id=223
 set @err_ret='Tato role je již obsazena v minimálnì jednom dokumentu ('+dbo.GetObjectAlias('p56',@x69recordpid)+')'
